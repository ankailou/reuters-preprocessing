...
def generate_dataset(documents, lexicon):
    ...
    for word in lexicon['title']:
        # UNCOMMENT FOR SANITY
        # print('Generating weights for word:', word)
        m.get_similarities(word, weights, 1.25)

    for word in lexicon['body']:
        # UNCOMMENT FOR SANITY
        # print('Generating weights for word:', word)
        if word not in lexicon['title']:
    ...
    print('Writing feature vector data @ dataset2.csv!')
    dataset = open("dataset2.csv", "w")
    ...
    for i, document in enumerate(documents):
        ...
        dataset.write('\n')
    dataset.close()
    print('Finished generating dataset @ dataset2.csv!')
